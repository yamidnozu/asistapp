...
class HorarioProvider extends ChangeNotifier with PaginatedDataMixin<Horario> {
  ...
  // Computed properties
  List<Horario> get horariosActivos =>
      items.where((horario) => horario.periodoAcademico.activo).toList();
  ...
  Future<void> loadHorarios(String accessToken,
  ...
  Future<void> loadHorariosByGrupo(String accessToken, String grupoId) async {
    ...
    try {
      debugPrint(
          'HorarioProvider: Iniciando carga de horarios por grupo $grupoId...');
      final horarios =
          await _horarioService.getHorariosPorGrupo(accessToken, grupoId);
      if (horarios != null) {
        debugPrint(
            'HorarioProvider: Recibidos ${horarios.length} horarios del grupo $grupoId');
        clearItems();
  ...
  Future<void> loadHorariosForGrupoWithConflictDetection(
    ...
    try {
      debugPrint(
          'HorarioProvider: Cargando horarios para grupo $grupoId y período $periodoId...');

      // Cargar ambas solicitudes en paralelo
      final grupoHorariosTask =
          _horarioService.getHorariosPorGrupo(accessToken, grupoId);
      ...
      if (periodResponse != null) {
        debugPrint(
            'HorarioProvider: Recibidos ${grupoHorarios?.length ?? 0} horarios del grupo');
        debugPrint(
            'HorarioProvider: Recibidos ${periodResponse.horarios.length} horarios del período');
        ...
        debugPrint(
            'HorarioProvider: Total horarios en memoria: ${items.length}');
      } else {
        setError('Error al cargar horarios');
      }
    } catch (e) {
      debugPrint(
          'HorarioProvider: Error loading horarios with conflict detection: $e');
      setError(e.toString());
    }
  }
  ...
  Future<void> loadHorarioById(String accessToken, String horarioId) async {
    ...
    try {
      final horario =
          await _horarioService.getHorarioById(accessToken, horarioId);
      if (horario != null) {
  ...
  Future<bool> createHorario(
    ...
    try {
      final newHorario =
          await _horarioService.createHorario(accessToken, horarioData);
      if (newHorario != null) {
        ...
        debugPrint(
            'createHorario: Horario creado pero sin objeto de retorno, recargando datos...');
        try {
  ...
  Future<bool> updateHorario(String accessToken, String horarioId,
    ...
    try {
      final updatedHorario = await _horarioService.updateHorario(
          accessToken, horarioId, horarioData);
      if (updatedHorario != null) {
        ...
        debugPrint(
            'updateHorario: Horario actualizado pero sin objeto de retorno, recargando datos...');
        try {
  ...
  Future<bool> deleteHorario(String accessToken, String horarioId) async {
    ...
    try {
      final success =
          await _horarioService.deleteHorario(accessToken, horarioId);
  ...
  Future<void> cargarClasesDelDia(String accessToken) async {
    ...
    try {
      debugPrint(
          'HorarioProvider: Cargando clases del día para el profesor...');
      final clases = await _horarioService.getMisClasesDelDia(accessToken);
      if (clases != null) {
        debugPrint(
            'HorarioProvider: Recibidas ${clases.length} clases del día');
  ...
  Future<void> cargarClasesPorDia(String accessToken, int diaSemana) async {
    ...
    try {
      debugPrint(
          'HorarioProvider: Cargando clases del día $diaSemana para el profesor...');
      final clases =
          await _horarioService.getMisClasesPorDia(accessToken, diaSemana);
      if (clases != null) {
        debugPrint(
            'HorarioProvider: Recibidas ${clases.length} clases del día $diaSemana');
        _clasesDelDia = clases;
        notifyListeners();
      } else {
  ...
  Future<void> cargarHorarioSemanal(String accessToken) async {
    ...
    try {
      debugPrint(
          'HorarioProvider: Cargando horario semanal para el profesor...');
      final horario = await _horarioService.getMiHorarioSemanal(accessToken);
      if (horario != null) {
        debugPrint(
            'HorarioProvider: Recibido horario semanal con ${horario.length} clases');
  ...
  List<ClaseDelDia> getHorarioPorDia(int diaSemana) {
    return _horarioSemanal
        .where((clase) => clase.diaSemana == diaSemana)
        .toList()
  ...
  Future<void> loadMoreHorarios(String accessToken,
      {String? grupoId, String? periodoId}) async {
  ...
  List<User> getProfesoresDisponibles(
    ...
    return allProfesors
        .where((profesor) => !profesoresConConflicto.contains(profesor.id))
        .toList();
  }
