...
void main() {
  ...
  Future<void> doLogout(WidgetTester tester) async {
    ...
    if (find.text('Iniciar Sesión').evaluate().isEmpty) {
      print('    ⚠️ Logout no efectivo. Drawer quizás abierto. Cerrando...');
      await tester.tapAt(const Offset(300, 300));
      await settle(tester);
      // Segundo intento
      if (find.text('Iniciar Sesión').evaluate().isEmpty) {
        final logoutIcon2 = find.byIcon(Icons.logout);
        if (logoutIcon2.evaluate().isNotEmpty) await tester.tap(logoutIcon2);
        await settle(tester, 2000);
      }
    }
  }
  ...
  testWidgets('📸 CAPTURA MAESTRA PART 6 (STUDENT + ACUDIENTE)',
    ...
    // Settings
    // Intentar buscar icon settings o menu
