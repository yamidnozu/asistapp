...
void main() {
  ...
  Future<void> goBack(WidgetTester tester) async {
    try {
      // Primero buscar botones de cancelar/cerrar explícitos si es dialog
      ...
      if (back.evaluate().isNotEmpty)
        ...
        // Si no encuentra, intentar tap fuera (para dialogs)
        // await tester.tapAt(const Offset(10, 10)); // Riesgoso
      }
      await settle(tester, 2000);
    } catch (_) {}
  }

  testWidgets('📸 CAPTURA MAESTRA PART 2 (ADMIN+)',
    ...
    if (find.byType(FloatingActionButton).evaluate().isNotEmpty) {
      ...
      await goBack(tester); // Cancelar form
    }
    await goBack(tester); // Volver a dashboard (Back de pantalla)
