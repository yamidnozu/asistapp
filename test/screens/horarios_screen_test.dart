...
class FakeHorarioProvider extends HorarioProvider {
  ...
  FakeHorarioProvider({this.initialHorarios = const []})
      : super(horarioService: _FakeHorarioService(initialHorarios));
...
class FakeHorarioConflictProvider extends FakeHorarioProvider {
  ...
  FakeHorarioConflictProvider(this.conflict,
      {super.initialHorarios = const []});
...
void main() {
  ...
  testWidgets('Create class via FAB opens dialog and calls createHorario',
                  ...
                  onPressed: () {
                    showDialog(
                        ...
                        builder: (_) => CreateClassDialog(
                            grupo: grupo, horaInicio: '07:00', diaSemana: 1));
                  },
  ...
  testWidgets('Create class conflict displays conflict dialog',
      (WidgetTester tester) async {
    final fakeAuth = FakeAuthProvider();
    fakeAuth.selectInstitution('i1');
    final periodo = PeriodoAcademico(
    ...
    final conflict = ConflictError(
        code: '409',
        reason: 'grupo_conflict',
        message: 'Conflict',
        meta: {
          'conflictingHorarioIds': ['h123']
        });
    ...
    expect(find.text('El grupo ya tiene una clase programada en este horario.'),
        findsOneWidget);
  });

  testWidgets('Edit class via dialog calls update and delete',
      (WidgetTester tester) async {
    final fakeAuth = FakeAuthProvider();
    final periodo = PeriodoAcademico(
        id: 'pX',
