...
// Seed maestro completo para AsistApp (JavaScript version)
// Sincronizado con seed.ts - Diciembre 2025
...
async function main() {
  ...
  console.log('✅ 3 períodos académicos creados.');
  ...
  const materiasSanJose = await Promise.all([
    prisma.materia.create({ data: { nombre: 'Cálculo', codigo: 'MAT-101', institucionId: colegioSanJose.id } }),
    prisma.materia.create({ data: { nombre: 'Física', codigo: 'FIS-101', institucionId: colegioSanJose.id } }),
    prisma.materia.create({ data: { nombre: 'Español', codigo: 'ESP-101', institucionId: colegioSanJose.id } }),
    prisma.materia.create({ data: { nombre: 'Inglés', codigo: 'ING-101', institucionId: colegioSanJose.id } }),
    prisma.materia.create({ data: { nombre: 'Química', codigo: 'QUI-101', institucionId: colegioSanJose.id } }),
  ]);

  const materiasSantander = await Promise.all([
    prisma.materia.create({ data: { nombre: 'Ciencias Sociales', codigo: 'SOC-101', institucionId: liceoSantander.id } }),
    prisma.materia.create({ data: { nombre: 'Matemáticas', codigo: 'MAT-101', institucionId: liceoSantander.id } }),
    prisma.materia.create({ data: { nombre: 'Arte', codigo: 'ART-101', institucionId: liceoSantander.id } }),
    prisma.materia.create({ data: { nombre: 'Educación Física', codigo: 'EFI-101', institucionId: liceoSantander.id } }),
  ]);
  ...
  const perfilAndres = await prisma.estudiante.create({
    data: {
      ...
      telefonoResponsable: TELEFONO_TEST,
      telefonoResponsableVerificado: false, // No verificado - para probar este caso
      aceptaNotificaciones: true,
    },
  });
  ...
  const perfilPaula = await prisma.estudiante.create({
    data: {
      ...
      telefonoResponsable: TELEFONO_TEST,
      telefonoResponsableVerificado: true,
      aceptaNotificaciones: false, // No acepta notificaciones - para probar este caso
    },
  });
  ...
  // ============================================================================
  // 11. CREAR HORARIOS COMPLETOS (TODOS LOS DÍAS L-V)
  // ============================================================================
  console.log('\n📅 Creando horarios semanales...');

  const horariosData = [];

  // Horarios Grupo 10-A San José
  for (let dia = 1; dia <= 5; dia++) {
    horariosData.push(
      ...
      },
      {
        ...
        horaInicio: '08:00',
        horaFin: '09:00',
        materiaId: materiasSanJose[1].id,
      ...
      },
      {
        ...
        horaInicio: '10:00',
        horaFin: '11:00',
        materiaId: materiasSanJose[2].id,
        profesorId: profesorJuan.id,
        grupoId: grupo10A.id,
        periodoId: periodoSanJose.id,
        institucionId: colegioSanJose.id,
      },
      {
        ...
        horaInicio: '14:00',
        horaFin: '15:00',
        materiaId: materiasSanJose[3].id,
        profesorId: profesorLaura.id,
        grupoId: grupo10A.id,
        periodoId: periodoSanJose.id,
        institucionId: colegioSanJose.id,
  ...
  for (let dia = 1; dia <= 5; dia++) {
    horariosData.push({
      ...
      horaInicio: '09:00',
      horaFin: '10:00',
      materiaId: materiasSanJose[4].id,
      profesorId: profesorLaura.id,
      grupoId: grupo11B.id,
      periodoId: periodoSanJose.id,
      institucionId: colegioSanJose.id,
  ...
  for (let dia = 1; dia <= 5; dia++) {
    horariosData.push(
      ...
      },
      {
        ...
        horaInicio: '08:00',
        horaFin: '09:00',
        materiaId: materiasSantander[1].id,
        profesorId: profesorCarlos.id,
        grupoId: grupo6_1.id,
        periodoId: periodoSantander.id,
        institucionId: liceoSantander.id,
    ...
    await prisma.asistencia.createMany({
      data: [
        ...
        // Asistencias de hace 3 días
        {
          ...
          estado: 'PRESENTE',
          horarioId: horarios.length > 1 ? horarios[1].id : horarios[0].id,
          estudianteId: perfilSantiago.id,
          profesorId: profesorLaura.id,
          institucionId: colegioSanJose.id,
          tipoRegistro: 'QR',
